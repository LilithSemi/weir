...
var media: Media = undefined;
var protocol: BlockIo = undefined;
...
/// simplefs.zig, which prepends it to the Hard Drive node.
pub const DISK_BASE = [20]u8{
    0x01, 0x04, 0x14, 0x00, // Hardware, Vendor, length 20
    0x57, 0x65, 0x69, 0x72, 0x44, 0x69, 0x73, 0x6b, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // "WeirDisk" GUID
};
...
fn readBlocks(self: *BlockIo, media_id: u32, lba: u64, buffer_size: usize, buf: [*]u8) callconv(.c) usize {
    _ = self;
    _ = media_id;
...
fn writeBlocks(self: *BlockIo, media_id: u32, lba: u64, buffer_size: usize, buf: [*]const u8) callconv(.c) usize {
    _ = self;
    _ = media_id;
    _ = lba;
    _ = buffer_size;
...
pub fn install() ?*handledb.Handle {
    ...
    if (h) |handle| {
        ...
        disk_dp[22] = 0x04;
        disk_dp[23] = 0x00;
        _ = handledb.install(handle, &DEVICE_PATH_GUID, @ptrCast(&disk_dp));
    }
    return h;
}

var part_media: Media = undefined;
var part_proto: BlockIo = undefined;
var part_dev: block.Partition = undefined;
...
fn partRead(self: *BlockIo, media_id: u32, lba: u64, buffer_size: usize, buf: [*]u8) callconv(.c) usize {
    _ = self;
    _ = media_id;
    const bs = part_dev.dev.block_size;
    if (buffer_size % bs != 0) return @intFromEnum(Status.bad_buffer_size);
    if (!part_dev.readBlocks(lba, @intCast(buffer_size / bs), buf[0..buffer_size])) return @intFromEnum(Status.device_error);
    return ok;
}
...
pub fn installPartition(handle: *handledb.Handle, part: block.Partition) void {
    ...
    _ = handledb.install(handle, &BlockIo.guid, @ptrCast(&part_proto));
}
