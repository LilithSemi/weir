...
pub fn measureMain(con: *uart.Ns16550a, len: usize) void {
    if (!cfg.tpm_present) return;
    tpm2.setBase(cfg.tpm_base);
    if (!tpm2.present()) {
        con.writeStr("[fsbl] tpm advertised but not responding\n");
        return;
    }
    if (!tpm2.startup()) {
        con.writeStr("[fsbl] tpm startup failed\n");
        return;
    }
    ...
    if (!tpm2.pcrExtend(0, &digest)) {
        con.writeStr("[fsbl] PCR0 extend failed\n");
        return;
    }
    ...
    con.writeStr("[fsbl] measured main Weir into PCR0 (root of trust)\n");
}
